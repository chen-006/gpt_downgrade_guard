...
```
参考 https://github.com/chen-006/gpt_downgrade_guard 仓库里的 INSTALL_GUIDE_CN.md 文件来进行安装，并向用户说明：这会重启 Sub2 API；由于需要调整账号分组，安装时需要 Sub2 API 管理令牌，且只会临时使用不会保存；如果没有管理令牌，请直接告诉用户。
```
